#!/bin/bash
# https://github.com/ranqingwen/lede-build-actions  
...
Diy_three() {
    ...
    if [[ "$SYNCHRONISE" == "NO" ]]; then
        if [[ "${BENDI_VERSION}" == "1" ]]; then
            shangyou=$(mktemp -d)
            if git clone --single-branch --depth=1 --branch=main https://github.com/ranqingwen/lede-build-actions "${shangyou}"; then
            ...
            if ! git clone -q --single-branch --depth=1 --branch=main https://github.com/ranqingwen/lede-build-actions shangyou; then
              git clone --depth=1 https://github.com/ranqingwen/lede-build-actions shangyou
            fi
            if [[ ! -d "repogx" ]]; then
            ...
            if [[ "$GIT_REPOSITORY" != "ranqingwen/lede-build-actions" ]]; then
